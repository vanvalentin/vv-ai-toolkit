...
def test_portable_skill_and_claude_adapter_share_the_name() -> None:
    canonical = (ROOT / ".agents" / "skills" / "job-offer-evaluation" / "SKILL.md").read_text(
        encoding="utf-8"
    )
    claude = (ROOT / ".claude" / "skills" / "job-offer-evaluation" / "SKILL.md").read_text(
        encoding="utf-8"
    )

    assert "name: job-offer-evaluation" in canonical
    assert "name: job-offer-evaluation" in claude
    assert "experts/job-offer-evaluation/methodology.md" in canonical
    assert "experts/job-offer-evaluation/methodology.md" in claude
