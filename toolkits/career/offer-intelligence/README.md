...
cd vv-ai-toolkit/toolkits/career
python3 offer-intelligence/scripts/setup.py
```

The script creates `.venv` inside the career toolkit, installs the package and
development checks in editable mode, and installs the managed Patchright Chromium
build. It is safe to rerun; an existing environment is reused.
...
with the career toolkit directory as its working directory. Restart or reload the client after
...
Open `toolkits/career/` as the workspace. Cursor expands `${workspaceFolder}` in
...
Start Claude Code from `toolkits/career/`. `.mcp.json` uses
...
Start Codex from `toolkits/career/` so the relative interpreter and `cwd` in
...
- smoke test manually with `.venv/bin/python -m offer_intel` from the career toolkit
  directory.

### Server exits or tools do not appear

- run `python -m offer_intel` manually and inspect standard error;
- verify the client opened `toolkits/career/` as its project root;
...
```bash
python offer-intelligence/scripts/check.py
```
